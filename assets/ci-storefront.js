...
(function () {
  ...
  var FLAVOR_LABEL = { fruit: 'Fruit & Floral', sweet: 'Sweet & Chocolate', terroir: 'Earthy & Complex', any: '' };
  var SHELF_BADGE = {
  ...
  window.filterTaste = function (el, axis, val) { activeTaste[axis] = val; setActiveAll(el); applyFilters(); };
  ...
  window.closeQuiz = function () { $('quiz-modal').classList.remove('active'); };
  ...
  window.showQuizResult = function () {
    var lbl = {
      ...
      flavor: { fruit: 'Fruit & Floral', sweet: 'Sweet & Chocolate', terroir: 'Earthy & Complex' },
  ...
  window.applyProfileAndClose = function () {
    ...
    $('profile-banner-tags').innerHTML = $('result-tags').innerHTML;
  ...
  window.simulateSignIn = function () {
    ...
    var btn = $('signin-btn'); btn.classList.add('signed-in');
    $('signin-label').textContent = session.name;
    closeSignin();
    renderCart();
    if (quizAnswers.roast || quizAnswers.flavor || quizAnswers.caffeine) { applyProfileAndClose(); }
  ...
  function renderCart() {
    ...
    var html = '';
    // subscriber sign-in banner (non-subscribers only)
    if (!session.signedIn) {
      html += '<div class="cart-banner"><span>Sign in for your subscriber benefits — 10% off Roccia, Sorpresa, and Selezione.</span><button onclick="openSignin()">Sign in</button></div>';
    }
    // discount math
  ...
  function renderAccount() {
    ...
    var tags = $('result-tags') ? $('result-tags').innerHTML : '';
    el.innerHTML =
      '<div class="acct-grid">' +
        ...
        '<div class="acct-card"><h3>Taste profile</h3>' +
          (tags ? '<div class="quiz-result" style="text-align:left;padding:0"><div class="tags" style="justify-content:flex-start">' + tags + '</div></div>' : '<p class="prose">Take the taste quiz to set your profile. <button class="skip-link" onclick="openQuiz()">Start</button></p>') +
      ...
      '</div>' +
      '<div class="section-head"><p class="eyebrow">Roccia subscription</p><h2>Manage your subscription</h2></div>' +
