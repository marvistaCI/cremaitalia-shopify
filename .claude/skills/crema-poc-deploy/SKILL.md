...
Then compare all files under `assets config layout locales snippets templates`. Expect **all 40 to
...
orphaned `assets/ci-cup.png`, then 38 → 39 when POC29 added Lauren Roberts' headshot
`assets/ci-lauren.jpg`, then 39 → 40 when POC30 added the hero video and its poster
`ci-temp-hero.mp4` / `ci-temp-hero-poster.jpg` and deleted the cafe band `ci-temp-lp1.jpg` — update
this number here whenever the theme's file set legitimately
