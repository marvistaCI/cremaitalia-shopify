...
ds["A2"] = "The decision layer behind the inventory. BUILD GATE means it must be true before the production build starts - those rows sit at the top. LOCKED means a Standard or CLAUDE.md sec9 records it. OPEN means it will force a spec revision if it lands the wrong way."
ds["A2"].font = sub_font
...
["BUILD GATE","Wholesale channel - in, out, and what the build must accommodate","Shopify / customer model / pricing","UNDECIDED. Whether Crema Italia sells to the trade at all, and if so whether as a RELATIONSHIP channel (a person sells, the order goes out as a Shopify draft order paid by card, no storefront) or a SELF-SERVICE one (trade login, gated catalogue, no shelves). Three options are set out in the brief; Option B, the relationship channel, is recommended because it forecloses nothing and costs almost no build.","Not the whole question blocks the build - three parts of it do, and each is rewrite-versus-edit: does a CUSTOMER carry a class (retail or trade), does a VARIANT carry a second price or a readable cost, and is a shelf the ONLY route to a product. All three are cheap now and expensive once the catalogue is built. Note a wholesale price is NOT a flat discount off retail - retail markup varies by shelf and size, so a flat percentage would give our thinnest margin on the 1 kg format cafes buy most of. Raised by the prospective Tampa cafe, but the channel stands or falls on its own: the cafe is one customer we would already control, and it needs an invoice rather than a login. UPSTREAM OF EVERYTHING: whether the roaster agreements permit wholesale at all. No roaster has been asked, and it belongs in the agreement rather than being discovered afterwards.","Gate set 2026-08-25","Operations\\In USA\\Tampa Operations\\Crema_Italia_Wholesale_Decision_v1.0.pdf (source: docs/wholesale/)"],
...
["RESOLVED","Roaster Guide archive pass - v1 to v6 archived, v6 renamed","Roaster Guide","DONE 2026-08-22. Guides v4, v5 and the v6 Italian review draft moved to archive with their editable .html masters alongside the .pdf renders, and the v6 files renamed off the _pending marker. v7 deliberately HELD in the working folder. Brief sec19 corrected: it had still been pointing at v7 as the current guide after v8 shipped.","The rename mattered because an archived file with a misleading name is still misleading - Lucia approved v6 on 2026-08-21 while the filename kept calling it pending. v7 was held back because Lucia has approved v6 but not v7 or v8, so v7 is the last fully reviewed pair and the fallback if v8 has to be withdrawn; archiving the fallback on the day you ship its replacement is the wrong order. Archiving the .html contradicts the v1-v3 precedent of PDF-only, deliberately - on this project the HTML is the master and the PDF only a render, so archiving the render alone discards the source. Verified by diff against a pre-run baseline: 11 files out, 11 in, nothing left the archive, one brief hunk.","2026-08-22","Cowork archive pass; diff-verified"],
...
["BUILD GATE","Account branding","Customer-account UI extensions","Does the accounts and checkout branding editor offer Marcellus?","Two minutes. Decides whether the hosted account surface diverges from the storefront on TYPE as well as layout.","Open","CLAUDE.md sec10 item B1"],
...
["BUILD GATE","Flow reliability","Shopify Flow","Shopify Flow for Offerta aging has never been tested. Liquid date comparison is unreliable in Flow.","The whole phased price-tool approach rests on Flow doing the aging transition.","Open","CLAUDE.md sec10 item C5"],
...
["BUILD GATE","3PL","Tampa 3PL","Not selected. Three QUALIFYING questions must be asked before commercial terms.","Offerta-vs-fresh segregation of the same SKU, our packing slip vs their paperwork, and per-order printed inserts. A 3PL that fails the third cannot fulfil Sorpresa at all.","Open","Store Operating Standards sec12.9"],
...
["BUILD GATE","Analytics stack","GA4 / Shopify Analytics","No decision recorded anywhere.","And Shopify's own discount reporting is structurally blind to the subscriber benefit, so it cannot be the answer on its own.","Open","No governing document"],
...
TRACK = {
 ...
 "Benefit visibility": ("Steve + Code", "Decide whether the theme renders the subscriber benefit from base-vs-plan price. It changes cart and product markup, so it is cheaper to decide than to retrofit.", "BEFORE BUILD STARTS"),
 ...
 "Account branding": ("Steve", "Settings > Checkout > Configurations > Edit - does the branding editor offer Marcellus, or accept an uploaded font? Two minutes, with the Browser pane displayed.", "BEFORE BUILD STARTS"),
 "Review records": ("Steve", "Ask Judge.me support the two questions in production_build_spec sec6.1.", "Before reviews go live"),
 "Flow reliability": ("Code", "Test a Flow scheduled trigger with Run-code date arithmetic on the dev store. Test the FAILURE MODE - the reported problem is that date comparison fails silently.", "BEFORE BUILD STARTS (its own stated deadline)"),
 ...
 "3PL": ("Steve", "Shortlist and ask the three QUALIFYING questions in Standard sec12.9. The ANSWERS gate the build; the contract does not.", "Answers: BEFORE BUILD STARTS. Contract: before launch."),
 ...
 "Does the site publish a phone number?": ("Steve + Code", "Decide whether the storefront carries a tel: link. Feeds the contact page and the JSON-LD contactPoint.", "BEFORE BUILD STARTS"),
 "Two cloud document stores": ("Steve", "Deliberate look at OneDrive vs Google Drive overlap - not a fix, a decision.", "At next renewal"),
 "Analytics stack": ("Steve", "Choose an analytics stack. It is a snippet in theme.liquid plus a consent posture, so it wants deciding before the layout is written, not after.", "BEFORE BUILD STARTS"),
 ...
 "Duplicate SKUs give INDEPENDENT stock pools - overselling, not just mis-picking": ("Steve + Code", "Re-open the three sec13.9.2 candidates against overselling, not just mis-picking. Decide once the 3PL answers land - it picks whether Offerta is a second product, a variant, or a tag.", "BEFORE BUILD STARTS - it is a data shape, not a page."),
 "Wholesale channel - in, out, and what the build must accommodate": ("Steve", "Decide between Option A (no wholesale), Option B (relationship channel - recommended) and Option C (self-service). If not A: ask the roasters BEFORE any of them signs, settle markup-versus-margin and the cost basis, and set the minimum order. If C is on the table at all: verify Shopify B2B plan gating and Liquid customer-object reliability on the dev store - both are short tests.", "BEFORE BUILD STARTS - it is a data shape, not a page."),
...
block("Status values", [
 ("BUILD GATE", "Decisions sheet only. Must be true BEFORE the production build starts, as distinct from before launch. Most open items on this sheet are launch-blocking, not build-blocking - waiting on those costs weeks and buys nothing. Build-blocking means: if this is wrong, we rewrite rather than edit."),
